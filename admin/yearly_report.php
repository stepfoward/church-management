...
// Function to get donations for a specified year
function getDonationsByYear($conn, $start_date, $end_date) {
    ...
    echo "<h3>Ripoti ya Michango kwa Mwaka</h3>";
    echo "</div>";
    ...
    if ($result->num_rows > 0) {
        ...
        echo "<div class='alert alert-warning text-center'>Hakuna michango iliyorekodiwa kwa mwaka huu.</div>";
    }
...
<head>
    ...
    <title>Ripoti ya Michango kwa Mwaka</title>
    <style>
        body {
...
<div class="container-fluid">
    <div class="row">
        <!-- Form ya kuchagua mwaka -->
        <div class="col-md-3 ms-auto mt-3">
            <form action="" method="GET" class="d-flex flex-column gap-3">
                <div class="d-flex gap-3">
                    <div class="mb-2 flex-grow-1">
                        <label for="year" class="form-label">Chagua Mwaka</label>
                        <input type="number" name="year" id="year" class="form-control" placeholder="Tarehe ya Mwaka (e.g. 2025)" required>
                    </div>
                </div>
                <button type="submit" class="btn btn-primary">Pata Ripoti ya Mwaka</button>
            </form>
        </div>

        <!-- Onyesha ripoti kulingana na mwaka -->
        <div class="col-md-10 ms-auto">
            <?php
            if (isset($_GET['year'])) {
                $year = $_GET['year'];
                $start_date = $year . '-01-01';  // Start date of the year
                $end_date = $year . '-12-31';    // End date of the year
                getDonationsByYear($conn, $start_date, $end_date);
            }
            ?>
        </div>
    </div>
